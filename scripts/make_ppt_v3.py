"""
TeamWorkHub Premium Presentation Generator v3
White + Red elegant theme, Apple Keynote style
"""
...
def add_section_label(slide, left, top, text, color=RED_600):
    """Add a small section label like 'PROBLEM' or 'FEATURE 01'."""
...
add_textbox(slide, Inches(0.9), Inches(4.2), Inches(8), Inches(0.6),
            "AI-powered email intelligence pipeline for your team",
...
add_title_with_line(slide, MARGIN_LEFT, Inches(1.0),
                    "Why email management is broken", Pt(30))

# 4 pain point cards
card_data = [
    {"icon": "01", "title": "Time Sink",
     "desc": "30+ minutes daily spent sorting,\nforwarding, and filing emails"},
    {"icon": "02", "title": "Lost Context",
     "desc": "Critical action items buried in\nlong reply chains (RE:RE:RE:)"},
    {"icon": "03", "title": "Zero Visibility",
     "desc": "No centralized view of who\nowns what across the team"},
    {"icon": "04", "title": "Manual Reports",
     "desc": "Weekly summaries assembled\nby hand every Monday morning"},
]
...
# ═══════════════════════════════════════════════════════════════════════
# SLIDE 3 — SOLUTION (Pipeline Flow)
# ═══════════════════════════════════════════════════════════════════════
slide = prs.slides.add_slide(blank_layout)
...
add_title_with_line(slide, MARGIN_LEFT, Inches(1.0),
                    "Fully automated pipeline", Pt(30))

# Subtitle
add_textbox(slide, MARGIN_LEFT, Inches(1.85), Inches(10), Inches(0.4),
            "From inbox to insight, without lifting a finger.",
            font_size=Pt(13), font_color=GRAY_500, italic=True)

# Flow diagram: 4 boxes with arrows
flow_items = [
    {"icon": "G", "title": "Gmail", "sub": "Fetch new\nemails"},
    {"icon": "AI", "title": "Claude AI", "sub": "Analyze &\ncategorize"},
    {"icon": "O", "title": "Obsidian", "sub": "Generate\nnotes"},
    {"icon": "R", "title": "Reports", "sub": "Daily/Weekly\nMonthly"},
]
...
add_textbox(slide, Inches(1.3), desc_bar_y + Inches(0.15), Inches(10.7), Inches(0.6),
            "Runs automatically every weekday at 08:00 KST via Cloud Scheduler.\n"
            "Zero manual intervention. Idempotent processing ensures no duplicates.",
...
add_title_with_line(slide, MARGIN_LEFT, Inches(1.0),
                    "AI-Powered Email Intelligence", Pt(30))

# Left side: 4 capability cards (2x2)
cap_data = [
    {"title": "Smart Summary", "desc": "Claude AI generates a concise\nKorean summary of each email"},
    {"title": "Auto-Assignee", "desc": "Detects responsible persons\nfrom names and job titles"},
    {"title": "Priority Scoring", "desc": "Classifies urgency:\nnormal / important / urgent"},
    {"title": "Category Tagging", "desc": "Auto-categorizes: report,\nrequest, notice, schedule, etc."},
]
...
add_textbox(slide, rc_x + Inches(0.3), rc_y + Inches(0.2), rc_w - Inches(0.6),
            Inches(0.3), "Reply Chain Extraction",
            font_size=Pt(13), font_color=RED_600, bold=True)

# Simulated email chain
chain_lines = [
    ("RE:RE:RE: Q4 Report Review", GRAY_400, False),
    ("RE:RE: Q4 Report Review", GRAY_400, False),
    ("RE: Q4 Report Review", GRAY_400, False),
    ("Q4 Report Review", RED_600, True),
]
...
add_textbox(slide, rc_x + Inches(0.3), rc_y + Inches(2.4), rc_w - Inches(0.6),
            Inches(0.8),
            "Only the latest, meaningful content\nis extracted and processed by AI.",
...
add_textbox(slide, Inches(1.3), Inches(5.98), Inches(10.7), Inches(0.4),
            "Powered by Claude Haiku 4.5  |  Single API call per email  |  <2s processing time",
...
add_title_with_line(slide, MARGIN_LEFT, Inches(0.85),
                    "Automated Daily Notes", Pt(30))

add_textbox(slide, MARGIN_LEFT, Inches(1.65), Inches(5), Inches(0.35),
            "Every morning, a complete daily briefing is ready in Obsidian.",
...
code_lines = [
    ...
    ("assignees: [박은진, 김철수, 이영희]", CODE_GREEN),
    ("categories: [보고, 요청, 공지]", CODE_GREEN),
    ("has_urgent: true", CODE_RED),
    ("---", CODE_COMMENT),
    ("", None),
    ("### Today's Work", CODE_YELLOW),
    ("#### To Do List", CODE_MAUVE),
    ("- [ ] [[Q4 Report Review]] #박은진", CODE_TEXT),
    ("  담당자:: 박은진 | 우선순위:: 긴급", CODE_RED),
    ("- [ ] [[Server Migration Plan]] #김철수", CODE_TEXT),
    ("  담당자:: 김철수 | 카테고리:: 요청", CODE_GREEN),
    ("- [x] [[Weekly Team Meeting]] #이영희", CODE_COMMENT),
    ("", None),
    ("#### Schedule", CODE_YELLOW),
    ("- 09:00 Q4 리뷰 회의", CODE_TEXT),
    ("- 14:00 서버 마이그레이션 논의", CODE_TEXT),
]
...
feat_items = [
    ...
    ("Wiki Links", "[[Subject]] links connect\nindividual email notes"),
    ("Assignee Tags", "#Name tags enable filtering\nby responsible person"),
    ("Inline Fields", "담당자:: / 우선순위:: / 카테고리::\nfor Dataview queries"),
    ("Recurring Tasks", "Day-specific routine tasks\nauto-populated by weekday"),
    ("Overnight Capture", "Emails from 18:00~08:59\nnever missed"),
]
...
add_title_with_line(slide, MARGIN_LEFT, Inches(1.0),
                    "Comprehensive Reporting Suite", Pt(30))

add_textbox(slide, MARGIN_LEFT, Inches(1.85), Inches(10), Inches(0.35),
            "Three levels of automated reporting, from daily to monthly overview.",
            font_size=Pt(12), font_color=GRAY_500, italic=True)
...
report_data = [
    {
        "title": "Weekly Report",
        "subtitle": "YYYY-WNN.md",
        "items": [
            "Weekly email statistics",
            "Unresolved task checklist",
            "Category breakdown",
            "Thread-grouped listing",
        ],
        "accent": RED_600,
    },
    {
        "title": "Monthly Report",
        "subtitle": "YYYY-MM Monthly.md",
        "items": [
    ...
    {
        "title": "Dashboard",
        "subtitle": "Dashboard.md",
        "items": [
            "Real-time Dataview queries",
            "Per-assignee pages",
            "Urgent item highlights",
...
add_textbox(slide, MARGIN_LEFT, Inches(6.3), Inches(11), Inches(0.35),
            "All reports are generated as Obsidian-compatible Markdown with Dataview inline fields.",
...
add_textbox(slide, proc_x + Inches(0.3), proc_y + Inches(0.1),
            Inches(3), Inches(0.25), "FastAPI Application (Cloud Run)",
...
add_textbox(slide, Inches(11.5), Inches(2.7), Inches(1.6), Inches(0.25),
            "08:00 KST", font_size=Pt(9), font_color=RED_600, bold=True,
            alignment=PP_ALIGN.CENTER)
...
add_textbox(slide, before_x + Inches(0.3), ba_y + Inches(0.2),
            Inches(2), Inches(0.3), "BEFORE",
            font_size=Pt(14), font_color=GRAY_400, bold=True)

before_items = [
    "Manually read and sort 30+ emails daily",
    "Action items lost in reply chains",
    "Weekly reports compiled by hand (2+ hours)",
...
add_textbox(slide, after_x + Inches(0.3), ba_y + Inches(0.2),
            Inches(2), Inches(0.3), "AFTER",
            font_size=Pt(14), font_color=RED_600, bold=True)

after_items = [
    "All emails auto-processed and categorized",
    "Smart reply-chain extraction, nothing lost",
    "Reports generated automatically (0 effort)",
    "Full team dashboard with assignee tracking",
]
...
metrics = [
    {"num": "30", "unit": "min saved/day", "desc": "Email processing\ntime eliminated"},
    {"num": "0", "unit": "missed items", "desc": "Complete capture\nof all action items"},
    {"num": "100%", "unit": "recorded", "desc": "Every email auto-filed\nand searchable"},
    {"num": "1-click", "unit": "reports", "desc": "Daily, weekly, monthly\ngenerated instantly"},
]
...
# Roadmap items
roadmap = [
    ("Phase 1", "Cloud Run Deployment", "Deploy to GCP with Cloud Scheduler\nfor automated daily execution"),
    ("Phase 2", "Jira Integration", "Connect email tasks directly\nto Jira issue creation"),
    ("Phase 3", "Slack Notifications", "Real-time alerts for urgent\nitems via Slack webhook"),
    ("Phase 4", "Analytics Dashboard", "Web-based analytics with\nhistorical trend visualization"),
]
...
# ─── Save ─────────────────────────────────────────────────────────────
output_path = "TeamWorkHub_v3.pptx"
