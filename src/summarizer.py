...
class AnalysisResult:
    ...
    description: str = ""     # 150자 이내 메일 한 줄 요약 (6하원칙)
...
_SUMMARY_PROMPT = """\
다음 업무 메일을 읽고 6하원칙(누가/무엇을/언제/어디서/왜/어떻게)에 충실하게 한국어 불릿포인트 3~5개로 요약해줘.
각 불릿은 1~2문장(30자 이상)으로 구체적으로 작성해. 반말 간결체 (존대 금지).
불릿1 [배경]: 누가 왜 이 메일을 보냈는지, 불릿2 [요청]: 누구에게 무엇을 요청하는지, 불릿3 [방법]: 어떻게/어디서 처리하는지, 불릿4 [일정]: 마감일·수치·일정.
...
{{
  ...
  "description": "이기정이 혜령 파트장에게 CM360 4월 캠페인 데이터 검증 결과 공유. 결측치 비율 2.3%로 이전 대비 개선됨. 금요일까지 피드백 요청.",
  "summary": ["- [배경] Hannah Zheng이 SOT 유형별 CM360 활용 방식에 대해 이기정에게 후속 질의", "- [요청] SECZ/SECH/SEAU 법인의 CM360·DV360 데이터 차이 확인 필요", "- [방법] CM360 데이터와 Platform 데이터를 비교하여 tag 누락 여부 점검", "- [일정] 5월 중 법인별 컨펌 후 일괄 반영 예정"],
  "assignees": ["이름1", "이름2"],
  "priority": "보통",
  "category": "일반",
...
- description: 메일 전체 내용을 150자 이내 한 문장~두 문장으로 요약. 6하원칙(누가→누구에게→무엇을→왜→어떻게→언제) 중 핵심 요소를 포함. 반말 간결체로 (존대 금지).
  - 나쁜 예: "SOT 로직 관련 인라인 답변 완료. 기정 차장의 확인과 혜진 이사의 답변 검토 필요." (누가 누구에게 왜 답변했는지 불명확)
  - 좋은 예: "Hannah Zheng이 이기정에게 SOT 유형별 CM360 활용 방식에 대해 인라인 답변. 법인별 CM360 사용 목적 확인 및 데이터 비교 요청." (누가→누구에게→무엇을→어떻게 명확)
- summary: 6하원칙(누가/무엇을/언제/어디서/왜/어떻게)에 충실한 한국어 불릿 3~5개. 각 불릿은 1~2문장(30자 이상). 앞에 "- " 붙여서. 반말 간결체로 작성 (존대·경어 금지, ~입니다/~됩니다/~합니다/~드립니다 사용 금지). 용건만 딱딱하게.
  - 불릿1 [배경/WHY]: 누가(발신자) 왜 이 메일을 보냈는지. 어떤 프로젝트/업무의 어떤 맥락에서 발생한 건인지 구체적으로.
  - 불릿2 [요청/WHAT+WHO]: 누구에게 무엇을 요청·공유·보고하는지. "A가 B에게 ~를 요청" 형태로 주어+대상+행위를 명확히.
  - 불릿3 [방법/HOW+WHERE]: 어떻게 처리해야 하는지, 어디서(어떤 시스템/플랫폼/환경) 작업하는지. 구체적 방법·절차·조건 포함.
  - 불릿4 [일정/WHEN]: 마감일·일정·수치·데이터 등 구체적 정보. 날짜가 없으면 현재 진행 상황.
  - 불릿5 (선택): 추가 참고사항·제약조건·주의점. 내용이 없으면 생략.
  - 나쁜 예: "- CM360 4월 캠페인 성과 보고서 초안이 첨부되어 있으며, 금주 금요일까지 검토 후 피드백 요청드립니다" (존대체, 주어 없음)
  - 나쁜 예: "- SOT 로직 관련 문의에 대한 인라인 답변 제공됨" (누가 누구에게인지 불명확, 내용이 막연)
  - 좋은 예: "- Hannah Zheng이 SOT 유형별 CM360 활용 방식에 대해 이기정에게 후속 질의. SECZ/SECH/SEAU 법인의 CM360·DV360 데이터 차이 확인 필요"
  - 좋은 예: "- 이기정이 법인별 SOT 비교 정리 파일 전달. CM360 미사용·Direct Buy 전용·성과측정 메인 3가지 유형으로 분류"
  - 좋은 예: "- CM360 vs Buying Platform 데이터 비교 결과, DSP도 BP 사용이 적합하다는 결론. SEF 기준 Impr 감소·Click 증가 확인됨"
  - 좋은 예: "- 5월 중 법인별 컨펌 후 업데이트 로직 일괄 반영 예정. 이동규와 함께 논의 후 정리 필요"
- assignees: 이 메일에서 실제로 조치/확인/대응이 필요한 핵심 담당자 1~2명만 (직함·호칭 제외). 수신자(To) 명단이나 CC 전체를 나열하지 마. 단순 참조·열람 대상자는 제외. 메일이 특정인에게 요청하는 내용이면 그 사람만 기록.
  ...
  메일에 언급된 사람이 위 목록에 없으면 (예: 외부 업체, 고객사 담당자) 그 사람은 assignee에 넣지 마. 비슷한 이름의 팀원으로 대체하지 마. 해당 업무를 우리 팀에서 담당하는 사람이 없으면 [].
  없으면 []
