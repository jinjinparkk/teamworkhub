...
def compose_monthly(
    ...
    lines.append("")
    team_assignee_counts = {k: v for k, v in assignee_counts.items() if k in TEAM_MEMBERS}
    if team_assignee_counts:
        lines.append("**담당자별:**")
        for name, cnt in sorted(team_assignee_counts.items(), key=lambda x: -x[1]):
